...
#ifdef ROS_FOUND
void publish_msg(ros::Publisher *pub, full_scan_data_st *scan_frame,
                 ros::Time start, double scan_time, std::string frame_id,
                 bool clockwise, double angle_min, double angle_max,
  ...
  double diff =
      scan_frame->data[point_nums - 1].angle - scan_frame->data[0].angle;
  ...
  scanMsg.intensities.assign(point_nums,
  ...
  for (int i = 0; i < point_nums; i++) {
    ...
    unsigned int index =
        (unsigned int)((angle - scanMsg.angle_min) / scanMsg.angle_increment);
    if (index < point_nums) {
...
void publish_msg(rclcpp::Publisher<sensor_msgs::msg::LaserScan>::SharedPtr &pub,
                 full_scan_data_st *scan_frame, rclcpp::Time start,
...
int main(int argc, char **argv) {
  ...
  ros::Publisher scan_pub =
      nh.advertise<sensor_msgs::LaserScan>(scan_topic, 10);
  ...
  if (port.empty()) {
    ...
    std::cout << "get serial port:" << port.c_str() << ", baudrate:" << baudrate
              << std::endl;
#ifdef ROS_FOUND
    while (ros::ok())
#elif ROS2_FOUND
...
#endif
    {
      ...
      if (ret) {
#ifdef ROS_FOUND
        publish_msg(&scan_pub, &scan_data, start_scan_time, scan_duration,
                    frame_id, clockwise, angle_min, angle_max, min_range,
                    max_range);
#elif ROS2_FOUND
        publish_msg(publisher, &scan_data, start_scan_time, scan_duration,
                    frame_id, clockwise, angle_min, angle_max, min_range,
                    max_range);
#endif
      }
